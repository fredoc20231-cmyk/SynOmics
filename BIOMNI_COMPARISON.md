...
- The **SynOmics** column lists *exact tool names from `server/tool_registry.ts`*
  (194 tools at time of writing). Every one is backed by a real engine and a
...
| **SynOmics** | **194** | egress-gated live DB clients | **every tool CI-gated against ground truth** |
...
its 194 tools runs real code checked against a known numeric answer in CI, and it
