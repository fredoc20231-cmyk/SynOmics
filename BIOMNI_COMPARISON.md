...
- The **SynOmics** column lists *exact tool names from `server/tool_registry.ts`*
  (222 tools at time of writing). Every one is backed by a real engine and a
...
| **SynOmics** | **222** | egress-gated live DB clients | **every tool CI-gated against ground truth** |
...
its 222 tools runs real code checked against a known numeric answer in CI, and it
