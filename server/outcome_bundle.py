...
# Mandatory report attribution + citation, rendered directly beneath every report
# title (product citation name: "Synapse"). Every report/document/article the
# platform emits carries these two lines.
ATTRIBUTION = (
    "All analyses and interpretations in this report were generated with **Synapse**, "
    "an integrated full-stack bioinformatics analytical platform."
)
CITATION = (
    "Fadiel A, *et al.* Synapse: an integrated full-stack bioinformatics analytical "
    "platform. 2026."
)
...
def build_bundle(
    ...
    *,
    ...
    md = [f"# {title}", "",
          ...
          f"*Generated by Synapse — tool `{tool}` — {ts}*", ""]
    ...
    html = [
        ...
        "</style></head><body>",
        f"<h1>{_esc(title)}</h1>",
        f'<p style="border-left:4px solid {PALETTE["secondary"]};padding-left:10px">'
        + _esc(ATTRIBUTION.replace("**", ""))
        + f'<br><strong>Citation:</strong> {_esc(CITATION.replace("**", "").replace("*", ""))}</p>',
        f"<p><em>Generated by Synapse — tool <code>{_esc(tool)}</code> — {ts}</em></p>",
